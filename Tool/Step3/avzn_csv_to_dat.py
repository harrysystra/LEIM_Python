...
def output_to_dat(
    df, path, avzn_emp_by_luti_path, avzn_hh_pop_by_luti_path, output_file_name
):
    df["Category 1"] = df["Category 1"].map("{:.5e}".format)
    df["Category 2"] = df["Category 2"].map("{:.5e}".format)
    df["Category 3"] = df["Category 3"].map("{:.5e}".format)
    df["Category 4"] = df["Category 4"].map("{:.5e}".format)
    ...
    with open(os.path.join(path, f"{output_file_name}.dat"), "w") as file:
        ...
        file.write(f" Emp Source: {avzn_emp_by_luti_path}\n")
        file.write(f" HH/Pop Source: {avzn_hh_pop_by_luti_path}\n")
        file.write(
            f" ========================================================================\n"
        )
        file.write(
        ...
        for i in range(len(df)):
            ...
            cat_1_str = str(df.iloc[i]["Category 1"]).ljust(11)
            cat_2_str = str(df.iloc[i]["Category 2"]).ljust(11)
            cat_3_str = str(df.iloc[i]["Category 3"]).ljust(11)
            cat_4_str = str(df.iloc[i]["Category 4"]).ljust(11)
            file.write(
